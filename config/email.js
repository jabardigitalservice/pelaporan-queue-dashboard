...
const {
  SUBJECT_NAME, TEXT_CASE, TEXT_HISTORY, QUEUE, EXPIRED_INFO, REGARDS_INFO
} = require('../utils/constant')
const { createLogStatus } = require('../utils/log')
...
const condition = async (err, jobId, res) => {
  if (err) {
    ...
    const set = {
      ...
      'message.email': `Email Sent ${res.response}`,
      job_status: 'Sent',
      job_progress: 100
    }
    await createLogStatus(jobId, set)
